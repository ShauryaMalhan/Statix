...
Phase 3 ships **HTTP ingest** (not gRPC): the agent POSTs the Phase 2 batch JSON; the gateway denormalizes rows and writes to ClickHouse via a RowBinary coalescer ([ADR 055](../adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md)).

**Enterprise constraints:** [enterprise-latency.md](enterprise-latency.md) · **Validation:** [phase3-validation.md](phase3-validation.md) · **ADR:** [005](../adr/ingest/005-non-blocking-ingest-pipeline.md) (historical Kafka path), [055](../adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md) (current)
...
Backpressure: `ch_healthy` false or mpsc full → `503` → agent circuit breaker → WAL ([ADR 054](../adr/ingest/054-phase11-wal-spillway.md)).
...
| `batch_id` | string | UUID v4 per flush ([ADR 017](../adr/ingest/017-batch-lineage-metadata.md)) |
...
The ingest handler builds gateway-local `MetricRow` via `MetricRow::from_ingest` and enqueues to the coalescer mpsc ([ADR 056](../adr/gateway/056-phase13-part2-ingest-zero-alloc.md)).
...
| `/ready` | GET | **Readiness:** `200` if `ch_healthy` + mpsc &lt;80% ([ADR 029](../adr/gateway/029-ready-channel-depth-gate.md), [055](../adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md)); else `503` |
| `/metrics` | GET | Prometheus text ([ADR 012](../adr/gateway/012-finops-api-prometheus-metrics.md)) |
...
Wire types: `statix_wire::IngestBatch` ([ADR 028](../adr/meta/028-finops-wire-and-agent-rename.md)).
...
Loaded by `config::Config::from_env()` ([ADR 030](../adr/gateway/030-finops-api-config-struct.md)). Writer tuning in `clickhouse_writer.rs` ([ADR 055](../adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md)).
...
| `STATIX_MPSC_DEPTH_SAMPLE_MS` | `1000` | Background mpsc depth gauge sampler period ([ADR 060](../adr/observability/060-phase10-golden-signal-saturation-metrics.md)) |
| `CLICKHOUSE_URL` | `http://localhost:8123` | Read + write client |
| `CLICKHOUSE_USER` / `CLICKHOUSE_PASSWORD` | `default` / empty | Auth |

## Read API

`GET /api/v1/workloads/summary?hours=<u64>` — default **24** hours. Aggregates over `statix.workload_metrics` (no `FINAL` on summary — same WAL double-count caveat as `total_execs`; billing uses `FINAL` on the table) ([ADR 027](../adr/gateway/027-api-read-path-clickhouse.md), [058](../adr/agent/058-phase14-cpu-usage-tracking.md)).
...
Optional host API: `make run-api` — never both on `:3000` ([ADR 009](../adr/deploy/009-finops-api-docker-compose.md)).

...
**Status:** Accepted  
**Date:** 2026-06-08  
**Context:** [PHASE_13_PART2_PLAYBOOK.md](../../../.cursor/skills/statix-ebpf-agent/PHASE_13_PART2_PLAYBOOK.md) — Part 1 ([ADR 055](../ingest/055-phase13-part1-kafka-removal-rowbinary.md)) shipped the mpsc coalescer + RowBinary writer but retained a double-buffer: `FlatRow` in `statix-wire` and `MetricRow` in the gateway, with a transient `Vec<FlatRow>` in the ingest handler and a second `Vec<MetricRow>` at flush time.
...
The mpsc is **not** replaced with a per-request `clickhouse::inserter`. One insert per HTTP request would create one ClickHouse part per agent batch → merge storms at scale. The coalescer fuses rows across requests into large parts ([ADR 055](../ingest/055-phase13-part1-kafka-removal-rowbinary.md)).
...
- **Follow-up:** Infra strip completed in [ADR 057](../deploy/057-phase13-part2-infra-kafka-strip.md).

## References

- [ADR 055](../ingest/055-phase13-part1-kafka-removal-rowbinary.md) — queue-less RowBinary ingest
- [ADR 054](../ingest/054-phase11-wal-spillway.md) — at-least-once + WAL replay
- [ADR 011](../storage/011-replacingmergetree-dedupe-identity.md) — dedup identity

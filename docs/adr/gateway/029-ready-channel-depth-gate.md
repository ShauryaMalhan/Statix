...
**Status:** Accepted  
**Date:** 2026-06-05  
**Context:** Phase 5 — load balancers should stop sending ingest when the gateway queue is nearly full, before `POST /ingest` returns `503` per row ([ADR 021](021-ingest-ready-probe.md), [ADR 014](../kafka-legacy/014-kafka-producer-env-tuning.md)).

## Decision

- **`AppState.kafka_channel_capacity`** — configured capacity from `STATIX_KAFKA_CHANNEL_SIZE` (same as `mpsc::channel` in `kafka::spawn_producer`; default **8192**, min **1024** — [ADR 014](../kafka-legacy/014-kafka-producer-env-tuning.md)).

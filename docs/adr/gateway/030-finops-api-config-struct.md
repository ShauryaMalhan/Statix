...
- **`statix-gateway/src/config.rs`** — `Config::from_env()` at the top of `main()` (before `env_logger::init()`). Crate renamed from `finops-api` ([ADR 035](../meta/035-phase7-workspace-restructure.md)).
...
- **Deferred:** Kafka tuning env (`STATIX_KAFKA_CHANNEL_SIZE`, etc.) remain in `kafka.rs` ([ADR 014](../kafka-legacy/014-kafka-producer-env-tuning.md)).

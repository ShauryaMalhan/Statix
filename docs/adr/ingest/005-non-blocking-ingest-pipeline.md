...
2. **API (`finops-api`)** — `POST /ingest` hard-gates `schema_version == 2` (`400` otherwise); denormalizes each workload row, `mpsc::try_send((Vec<u8>, Vec<u8>))` to bounded channel (env, default 8192 — [ADR 014](../kafka-legacy/014-kafka-producer-env-tuning.md)); node key once per batch ([ADR 010](../kafka-legacy/010-kafka-partition-key-by-node.md)). `200 OK` when all rows enqueue; first `try_send` failure → `503`. `GET /metrics` — [ADR 012](../gateway/012-finops-api-prometheus-metrics.md).
3. **Kafka** — micro-batch (`recv_many`, env linger/batch_max); group by partition; `produce()` per partition sub-batch — [ADR 014](../kafka-legacy/014-kafka-producer-env-tuning.md).

Infrastructure: Kafka KRaft → ClickHouse `Kafka` engine table → materialized view → **`ReplacingMergeTree`** ([ADR 007](../storage/007-clickhouse-mergetree-tuning.md), [ADR 011](../storage/011-replacingmergetree-dedupe-identity.md)). No Rust consumer.
...
- **Ops:** `make compose-up` — [ADR 009](../deploy/009-finops-api-docker-compose.md). Agent: `STATIX_INGEST_URL=http://127.0.0.1:3000/ingest`.

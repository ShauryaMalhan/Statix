...
**Date:** 2026-05-28  
**Context:** Phase 3 plan noted that `reqwest::Client::new()` per batch creates a new connection pool each flush. Agent retries must not double-bill when K8s labels change between attempts ([ADR 011](../storage/011-replacingmergetree-dedupe-identity.md)).

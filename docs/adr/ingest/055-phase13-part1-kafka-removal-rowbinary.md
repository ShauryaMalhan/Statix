...
- `AppState`: `ingest_tx: mpsc::Sender<MetricRow>`, `ch_healthy: Arc<AtomicBool>`. *(Initial Part 1 shipped with `FlatRow`; collapsed to `MetricRow` in Part 2 — [ADR 056](../gateway/056-phase13-part2-ingest-zero-alloc.md).)*
...
- **Negative:** Gateway is terminal buffer — must fast-fail `503`. Infra strip completed in [ADR 057](../deploy/057-phase13-part2-infra-kafka-strip.md).
...
- [ADR 054](054-phase11-wal-spillway.md) — agent WAL + circuit breaker
- [ADR 056](../gateway/056-phase13-part2-ingest-zero-alloc.md) — single `MetricRow`; `FlatRow` removed

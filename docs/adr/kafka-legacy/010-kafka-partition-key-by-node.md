...
4. **Routing:** `FxHasher` over `node` UTF-8 bytes (`&[u8]`) `% num_partitions` → partition slot (amended V2-12 / [ADR 039](../fixes/039-phase55-v2-wave2-l8-fixes.md); was `DefaultHasher`). Record **key** / **value** own the queued buffers at produce time; key allocated once per partition chunk (V2-13).

...
- **F7:** Operational dashboard reads avoid `FINAL` merge cost; `argMax` dedupes by latest `window_start_ns` per cgroup/pod tuple. Billing exports still use `FINAL` ([ADR 011](../storage/011-replacingmergetree-dedupe-identity.md)).

## Consequences

- **Positive:** Lower agent memory churn on ingest retries; lower gateway CPU on Kafka produce and CH summary queries.
- **Negative:** Summary `sum(exec_count)` may briefly over-count duplicate rows before background merge — acceptable for dashboards ([ADR 027](../gateway/027-api-read-path-clickhouse.md) operational vs billing split).

...
- **`docker-compose.yml`** — `statix-gateway` service on port `3000`, `depends_on: kafka: service_healthy`, Kafka healthcheck via `kafka-broker-api-versions.sh`. Dev image: root `Dockerfile.gateway` ([ADR 035](../meta/035-phase7-workspace-restructure.md)).
...
- **ClickHouse init:** `deploy/clickhouse/01_init.sql` mounted in `docker-compose.yml` ([ADR 026](../storage/026-clickhouse-finops-database-init.md)).
- **Read-path env on `statix-gateway`:** `CLICKHOUSE_URL=http://clickhouse:8123`, `CLICKHOUSE_PASSWORD` from `.env` ([ADR 027](../gateway/027-api-read-path-clickhouse.md), [046](046-secrets-env-file.md)).

...
pub const TOPIC: &str = "statix-telemetry";
...
/// Configured ingest `mpsc` capacity (`STATIX_KAFKA_CHANNEL_SIZE`, default [`DEFAULT_CHANNEL_SIZE`], min [`MIN_CHANNEL_SIZE`]).
pub fn ingest_channel_capacity() -> usize {
    read_env_usize("STATIX_KAFKA_CHANNEL_SIZE", DEFAULT_CHANNEL_SIZE).max(MIN_CHANNEL_SIZE)
}

fn read_kafka_batch_max() -> usize {
    read_env_usize("STATIX_KAFKA_BATCH_MAX", DEFAULT_BATCH_MAX).clamp(64, 16_384)
}

fn read_kafka_linger() -> Duration {
    let ms = read_env_u64("STATIX_KAFKA_LINGER_MS", DEFAULT_LINGER_MS).clamp(1, 1000);
...
pub fn on_kafka_enqueued() {
    metrics::gauge!("statix_api_kafka_channel_depth").increment(1.0);
}

#[inline]
fn on_kafka_dequeued(count: usize) {
    if count > 0 {
        metrics::gauge!("statix_api_kafka_channel_depth").decrement(count as f64);
    }
}
...
pub fn spawn_producer(brokers: String) -> KafkaProducer {
    ...
    log::info!(
        "Kafka ingest mpsc capacity={channel_size} (STATIX_KAFKA_CHANNEL_SIZE, min {MIN_CHANNEL_SIZE})"
    );
    ...
    for (pid, mut rows) in by_partition.drain() {
        ...
        while !rows.is_empty() {
                ...
                .await
            {
                ...
                metrics::counter!("statix_api_kafka_produce_errors_total").increment(1);
                if failed_batches.len() < MAX_FAILED_BATCHES {
                    ...
                    metrics::counter!("statix_api_kafka_produce_dropped_total")
                        .increment(n as u64);
                }
                refresh_partition_metadata(client, partition_ids, clients).await;
            }
            metrics::histogram!("statix_api_kafka_produce_duration_seconds")

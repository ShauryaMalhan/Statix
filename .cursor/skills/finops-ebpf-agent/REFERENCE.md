...
- **Gateway env:** `config::Config::from_env()` in `finops-gateway/src/config.rs` — `KAFKA_BROKERS`, `FINOPS_API_PORT` (invalid → exit 1), `FINOPS_API_TOKEN`, `CLICKHOUSE_*` ([ADR 030](../../../docs/adr/030-finops-api-config-struct.md)); Kafka tuning in `kafka.rs` via `finops_infra::env` ([ADR 035](../../../docs/adr/035-phase7-workspace-restructure.md))

...
- [ ] BPF: EVENTS map name matches loader; reserve → fill → submit(0); on reserve fail increment `RING_DROPS` ([ADR 022](../../../docs/adr/ebpf/022-bpf-ring-buffer-drop-counter.md))
- [ ] Agent: no await on ring-buffer path; `DRAIN_BUDGET=256` ([ADR 032](../../../docs/adr/fixes/032-phase55-l8-p0-hot-path-fixes.md)); `emit_batch` moves `BatchPayload`; Prometheus `:9091` ([ADR 023](../../../docs/adr/fixes/023-phase5-hot-path-fixes.md))
- [ ] Aggregator: FxHashMap, double buffer, early flush (never enforce_cap); window bounds via `wall_unix_ns()` — never a cached offset ([ADR 063](../../../docs/adr/agent/063-wall-clock-window-bounds.md))
- [ ] Output: `STATIX_INGEST_URL` → `init_http_client` (+ optional `STATIX_API_TOKEN`) + `init_retry_worker` ([ADR 006](../../../docs/adr/ingest/006-shared-http-client-for-ingest.md), [ADR 019](../../../docs/adr/gateway/019-ingest-bearer-token-auth.md))
- [ ] API: `Config::from_env()` first in `main` ([ADR 030](../../../docs/adr/gateway/030-finops-api-config-struct.md)); GET /health; GET /ready (`ch_healthy` + mpsc &lt;80%); POST /ingest `try_reserve_many`; read API
...
| `statix-wire` | host lib | `IngestBatch`, `WorkloadRow` ([ADR 028](../../../docs/adr/meta/028-finops-wire-and-agent-rename.md)) |
| `statix-ebpf` | `bpfel-unknown-none` | tracepoint, `cgroup_id`, ring buffer (`STATIX_RING_BUF_BYTES` / [ADR 013](../../../docs/adr/ebpf/013-configurable-ring-buffer-size.md)) |
| `statix` | host | loader, attribution, aggregator, output; **`:9091/metrics`** ([ADR 022](../../../docs/adr/ebpf/022-bpf-ring-buffer-drop-counter.md), [ADR 023](../../../docs/adr/fixes/023-phase5-hot-path-fixes.md)) |
| `statix-gateway` | host | `clickhouse_writer` RowBinary coalescer; `MetricRow::from_ingest`; ingest + read API; dashboard (`routes/dashboard/`, [ADR 061](../../../docs/adr/ui/061-phase15-dashboard-read-tier.md)); probes ([ADR 021](../../../docs/adr/gateway/021-ingest-ready-probe.md), [ADR 029](../../../docs/adr/gateway/029-ready-channel-depth-gate.md), [055](../../../docs/adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md), [056](../../../docs/adr/gateway/056-phase13-part2-ingest-zero-alloc.md)) |
| `statix-infra` | lib | `read_env_positive` (via `read_env_u64`/`read_env_usize`), clock helpers ([ADR 035](../../../docs/adr/meta/035-phase7-workspace-restructure.md), [048](../../../docs/adr/meta/048-generic-env-positive-parsing.md)) |

**Infra:** `docker-compose.yml` (ClickHouse, Grafana `:3001`, gateway), `deploy/docker/`, `deploy/k8s/`, `deploy/clickhouse/01_init.sql` ([ADR 057](../../../docs/adr/deploy/057-phase13-part2-infra-kafka-strip.md))
...
| `emit_batch` | Serialize + `try_send` to retry worker; on full queue, `try_append` to disk WAL spillway (Phase 11, [ADR 054](../../../docs/adr/ingest/054-phase11-wal-spillway.md)) then last-resort sync drop-oldest; backoff + jitter; 0–5s recovery jitter after outage ([ADR 006](../../../docs/adr/ingest/006-shared-http-client-for-ingest.md), [042](../../../docs/adr/fixes/042-phase55-v2-p2-sprint-l8-fixes.md)) |
...
| `POST /ingest` | `schema_version` 2 or 3 or `400` ([ADR 020](../../../docs/adr/ingest/020-ingest-schema-version-window.md)); Tier 1 `!ch_healthy`→503; Tier 2 `try_reserve_many`→503; 2MB body limit ([ADR 052](../../../docs/adr/fixes/052-phase55-v3-wave4-perf-observability.md)) |
| ClickHouse writer | Background task only — `MetricRow` coalescer; RowBinary micro-batches; sync `insert.end()` ACK ([ADR 055](../../../docs/adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md), [056](../../../docs/adr/gateway/056-phase13-part2-ingest-zero-alloc.md)) |
...
| CPU sample | Same tick: `cpu.stat` cumulative counter → delta in `Sampler.cpu_baseline`; prime first read ([ADR 058](../../../docs/adr/agent/058-phase14-cpu-usage-tracking.md)). At startup `Sampler::prime` sets every leaf's baseline and the first flush is one full window out (`interval_at`), so the first window has CPU ([ADR 069](../../../docs/adr/agent/069-prime-cpu-and-full-first-window.md)) |
| Sample timing | Every window closes through `sample_and_flush` (flush timer **and** SIGINT/SIGTERM) — sample, then `flush`, exactly one reading per window. Never call `agg.flush` directly; never a separate timer: `select!` picks ready branches at random → windows with 0 or 2 samples ([ADR 067](../../../docs/adr/agent/067-sample-inside-flush.md)) |
...
- No `bpf_trace_printk`
- `submit(wakeup_flag)` — `BPF_RB_NO_WAKEUP` (flag `1`) on 63/64 events; 5ms poll drain in agent ([ADR 053](../../../docs/adr/fixes/053-phase55-v3-wave5-micro-arch-polish.md))
- `cgroup_id` from `bpf_get_current_cgroup_id()` on identity events
- **CI matrix (BTF-era only):** Linux **5.10, 5.15, 6.1, 6.8** (mainline LTS tips, not `.0`) — [ADR 037](../../../docs/adr/ebpf/037-phase9-ebpf-verifier-ci.md); `.github/workflows/ebpf-ci.yml`; `scripts/verify-ebpf-kernel.sh` + `statix-ebpf-verify`
...
- **Attribution hot path (V2):** `on_identity_event` procfs skip when cgroup known; K8s label merge outside write lock ([ADR 039](../../../docs/adr/fixes/039-phase55-v2-wave2-l8-fixes.md))
- **Kafka routing (V2, historical):** removed with `kafka.rs` ([ADR 055](../../../docs/adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md); was [ADR 039](../../../docs/adr/fixes/039-phase55-v2-wave2-l8-fixes.md))
- **Kafka durability (V2, historical):** removed with `kafka.rs` ([ADR 055](../../../docs/adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md); was [ADR 040](../../../docs/adr/fixes/040-phase55-v2-wave3-l8-fixes.md))
- **K8s eviction (V2):** agent + gateway `preStop sleep 5` + `terminationGracePeriodSeconds: 30`; gateway PDB `minAvailable: 1` ([ADR 040](../../../docs/adr/fixes/040-phase55-v2-wave3-l8-fixes.md))
- **K8s labels (V2):** `watch_k8s_pods` — `kube::runtime::watcher` with node field selector; no 30s list poll ([ADR 041](../../../docs/adr/fixes/041-phase55-v2-wave4-l8-fixes.md))
- **K8s deploy (V2):** digest-pinned images; gateway cross-AZ `topologySpreadConstraints` ([ADR 041](../../../docs/adr/fixes/041-phase55-v2-wave4-l8-fixes.md))
...
| `HashMap` for `cgroup_id` | `FxHashMap` ([ADR 001](../../../docs/adr/agent/001-use-rustc-hash-for-latency.md)) |
...
| Rule | Detail |
|------|--------|
| Locks | `parking_lot::RwLock`; **procfs before `write()`** on identity ([ADR 023](../../../docs/adr/fixes/023-phase5-hot-path-fixes.md)) |
...
| Component | Rule |
|-----------|------|
| Agent | `init_http_client`; `init_retry_worker`; circuit breaker + WAL on 503 ([ADR 054](../../../docs/adr/ingest/054-phase11-wal-spillway.md), [006](../../../docs/adr/ingest/006-shared-http-client-for-ingest.md)) |
| API | `GET /health` (liveness); `GET /ready` = `ch_healthy` + mpsc &lt;80% ([ADR 021](../../../docs/adr/gateway/021-ingest-ready-probe.md), [ADR 029](../../../docs/adr/gateway/029-ready-channel-depth-gate.md), [055](../../../docs/adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md)); `POST /ingest` Tier 1/2 503; `statix_api_ingest_lag_seconds` |
| Writer | `clickhouse_writer.rs` — `MetricRow` coalescer → RowBinary INSERT; `MetricRow::from_ingest` in handler ([ADR 056](../../../docs/adr/gateway/056-phase13-part2-ingest-zero-alloc.md)); `STATIX_CH_*` env; no `async_insert` |
| CH storage | `statix.workload_metrics`; `ReplacingMergeTree(window_end_ns)`; `INDEX cgroup_idx` minmax on `cgroup_id`; billing `FINAL`; init [deploy/clickhouse/01_init.sql](../../../deploy/clickhouse/01_init.sql) ([ADR 007](../../../docs/adr/storage/007-clickhouse-mergetree-tuning.md), [026](../../../docs/adr/storage/026-clickhouse-finops-database-init.md), [059](../../../docs/adr/storage/059-phase10-clickhouse-cgroup-skip-index.md)) |
| Prod deploy | `deploy/docker/Dockerfile.{gateway,agent}`; `deploy/k8s/*.yaml` ([ADR 024](../../../docs/adr/deploy/024-agent-production-container.md), [025](../../../docs/adr/deploy/025-kubernetes-gateway-and-agent.md)) |
...
**P0-SHIP shipped:** [ADR 032](../../../docs/adr/fixes/032-phase55-l8-p0-hot-path-fixes.md) — agent hot path.

**P1-WEEK shipped:** [ADR 033](../../../docs/adr/fixes/033-phase55-l8-p1-week-gateway-fixes.md) — `Bytes` retry body, Kafka producer alloc fixes, cached `kube::Client`, metadata refresh, `argMax` summary query.

**P2-SPRINT shipped (historical):** [ADR 034](../../../docs/adr/fixes/034-phase55-l8-p2-ingest-zero-copy.md) — superseded by gateway `MetricRow` path ([ADR 056](../../../docs/adr/gateway/056-phase13-part2-ingest-zero-alloc.md)).

**L8 playbook:** [L8-AUDIT-FIXES.md](L8-AUDIT-FIXES.md) — all fixes shipped (ADR index).

**L8 V2 playbook:** [L8_AUDIT_V2_FIXES.md](L8_AUDIT_V2_FIXES.md) — all V2 items shipped for GA ([ADR 038](../../../docs/adr/fixes/038-phase55-v2-wave1-l8-fixes.md)–[042](../../../docs/adr/fixes/042-phase55-v2-p2-sprint-l8-fixes.md)).

**L8/L9 V3 playbook:** [L8_POST_GA_FIXES.md](L8_POST_GA_FIXES.md) — all V3 waves shipped ([ADR 049](../../../docs/adr/fixes/049-phase55-v3-wave1-silent-deaths.md)–[053](../../../docs/adr/fixes/053-phase55-v3-wave5-micro-arch-polish.md)).

**Phase 13 playbooks:** [PHASE_13_PART1_PLAYBOOK.md](PHASE_13_PART1_PLAYBOOK.md) · [PHASE_13_PART2_PLAYBOOK.md](PHASE_13_PART2_PLAYBOOK.md) — [ADR 055](../../../docs/adr/ingest/055-phase13-part1-kafka-removal-rowbinary.md)–[057](../../../docs/adr/deploy/057-phase13-part2-infra-kafka-strip.md).

**Phase 14 playbook:** [PHASE_14_CPU_PLAYBOOK.md](PHASE_14_CPU_PLAYBOOK.md) — shipped ([ADR 058](../../../docs/adr/agent/058-phase14-cpu-usage-tracking.md)).

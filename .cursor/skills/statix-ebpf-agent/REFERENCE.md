...
- Agent event loop: `watch_k8s_pods` stream; sampler reads `memory.current` + `cpu.stat` of leaf cgroups only in one `spawn_blocking`/tick ([ADR 058](../../../docs/adr/agent/058-phase14-cpu-usage-tracking.md), [066](../../../docs/adr/agent/066-sample-leaf-cgroups-only.md)); ingest retry = `bytes::Bytes`

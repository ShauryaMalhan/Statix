...
- Agent event loop: `watch_k8s_pods` stream; sampler reads `memory.current` + `memory.stat` (working set, [ADR 071](../../../docs/adr/agent/071-working-set-memory.md)) + `cpu.stat` of leaf cgroups only in one `spawn_blocking`/tick ([ADR 058](../../../docs/adr/agent/058-phase14-cpu-usage-tracking.md), [066](../../../docs/adr/agent/066-sample-leaf-cgroups-only.md)); ingest retry = `bytes::Bytes`
...
- Startup cgroup bootstrap: `bootstrap_existing_cgroups` (walkdir + dir `ino()` = `cgroup_id`; `STATIX_CGROUP_ROOT`; registers paths only, no aggregator events) — [ADR 015](../../../docs/adr/agent/015-cgroup-v2-bootstrap-on-startup.md), [068](../../../docs/adr/agent/068-bootstrap-registers-only.md)
